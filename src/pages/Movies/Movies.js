...
function Movies() {
  ...
  useEffect(() => {
    const getMovies = async () => {
      await getTopRatedMovie(1).then((res) => {
  ...
  return (
    <div className="movies-page">
      <GenresMovies />
      <HeroSlider title="Popular movie" movies={state.moviesPopular} />
      <HeroSlider title="Top movie rated" movies={state.moviesRated} />
    </div>
  );
}
